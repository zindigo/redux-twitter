...
class TweetCard extends React.Component {
	...
	render() {
		const { tweet, users } = this.props
		const { id, author, timestamp, replyingTo, text, replies, likes } = this.props.tweet

		return (
		    <div>
	    		<Link className='tweet' to={`/tweet/${id}`}>
	    		<img src={users[author].avatarURL} alt='Avatar of {users[author].name}' className='avatar' />
	    		<div className='tweet-info'>
	    			<div>
		    			...
		    			{ replyingTo &&
		    			<button className='replying-to' onClick={(e) => this.handleReplyToClick(e, replyingTo)}>
		    				Replying to @{this.getReplyToTweet(replyingTo).author}
		    			</button>
		    			}
		    			<p>{text}</p>
	    			</div>
